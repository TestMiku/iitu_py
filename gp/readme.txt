...
1. В "settings.py" можно изменять  INSTALLED_APPS добавлять свои новые созданные apps после "#gp's apps"
2. Другие изменения в "settings.py" нужно согласовать с коммандой
3. У каждого разраба есть своя ветка (gp, np, mp)  в котором есть urls.py. по ссылке "host:post/np/" будут попадаться в вашу ветку. Все ссылки на ваши приложении прописывать там
4. В папках "media/", "templates/", "static/" - хранить все элементы в своих папках внутри этих папок (на пример:
    "media/gp/", "templates/gp/", "static/gp/")
